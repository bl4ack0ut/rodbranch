...
type IOutcomeModalProps = {
  modalIsOpen: boolean;
  outcomes: Array<{ message: string; source: string; linkMessage?: string; link?: string }>;
  closeModal: () => void;
};
export const OutcomeModal: React.FunctionComponent<IOutcomeModalProps> = ({
  modalIsOpen,
  outcomes,
  closeModal,
}) => {
      ...
      <Container>
        <div className="w-full flex flex-col items-center overflow-hidden" style={{ zIndex: 1000 }}>
          ...
          <div className="w-full flex items-center justify-around ">
            {outcomeIndex > 0 ? (
              <div onClick={() => previous()} className="cursor-pointer mr-3">
                <img
            ...
            {outcomeIndex < outcomes.length - 1 ? (
              <div className="cursor-pointer ml-3" onClick={() => next()}>
                <img
